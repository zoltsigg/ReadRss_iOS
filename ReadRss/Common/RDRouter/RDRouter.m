...
static NSString * const ROUTER_WILDCARD_CHARACTER = @"~";
static NSString *specialCharacters = @"/?&.";

NSString *const RouterParameterURL = @"RouterParameterURL";
NSString *const RouterParameterCompletion = @"RouterParameterCompletion";
NSString *const RouterParameterUserInfo = @"RouterParameterUserInfo";
...
+ (void)registerURLPattern:(NSString *)URLPattern toHandler:(RouterHandler)handler
{
...
+ (void)openURL:(NSString *)URL withUserInfo:(NSDictionary *)userInfo completion:(void (^)(id result))completion
{
    ...
    if (parameters) {
        RouterHandler handler = parameters[@"block"];
        if (completion) {
            parameters[RouterParameterCompletion] = completion;
        }
        if (userInfo) {
            parameters[RouterParameterUserInfo] = userInfo;
        }
        if (handler) {
...
{
    RDRouter *router = [RDRouter sharedInstance];
    
    URL = [URL stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    NSMutableDictionary *parameters = [router extractParametersFromURL:URL matchExactly:NO];
    RouterObjectHandler handler = parameters[@"block"];
...
+ (void)registerURLPattern:(NSString *)URLPattern toObjectHandler:(RouterObjectHandler)handler
{
    [[self sharedInstance] addURLPattern:URLPattern andObjectHandler:handler];
}

- (void)addURLPattern:(NSString *)URLPattern andHandler:(RouterHandler)handler
{
...
- (void)addURLPattern:(NSString *)URLPattern andObjectHandler:(RouterObjectHandler)handler
{
    NSMutableDictionary *subRoutes = [self addURLPattern:URLPattern];
    if (handler && subRoutes) {
        subRoutes[@"_"] = [handler copy];
...
- (NSMutableDictionary *)extractParametersFromURL:(NSString *)url matchExactly:(BOOL)exactly
{
    ...
    parameters[RouterParameterURL] = url;
    ...
    for (NSString* pathComponent in pathComponents) {
        ...
        for (NSString* key in subRoutesKeys) {
            if ([key isEqualToString:pathComponent] || [key isEqualToString:ROUTER_WILDCARD_CHARACTER]) {
                found = YES;
...
- (NSArray*)pathComponentsFromURL:(NSString*)URL
{
    ...
    if ([URL rangeOfString:@"://"].location != NSNotFound) {
        ...
        if (!URL.length) {
            [pathComponents addObject:ROUTER_WILDCARD_CHARACTER];
        }
    }
